...
def get_all_pdf(path):
    ...
    for pdf in pdf_list:
        pdf_temp = Paper(pdf)
        # pdf_temp.parse_pdf() if not pdf_temp.pdf else None
        text = ''
        text += 'Title:' + pdf_temp.title
        # text += 'Url:' + paper.url
        text += ' Abstract:' + pdf_temp.abs
        text += ' Paper_info:' + pdf_temp.section_text_dict['paper_info']

        for key, value in pdf_temp.section_text_dict.items():
            if key != 'paper_info' or key != 'Abstract' or key != 'title':
                text += " {}:{}".format(key, value)
        pdf_result_list.append(text)
        pdf_title_list.append(pdf_temp.title)
    return pdf_result_list, pdf_title_list
